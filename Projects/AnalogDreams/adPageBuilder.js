$().ready(function(){
    $.getJSON("/Projects/AnalogDreams/adData.json",function(data){
        ...
        for(i in data.Kits)
        {
            ...
            $("#kits").append('<img class="kitRender hover-shadow" onclick="openModal();currentSlide('+num+')" src="/Project/AnalogDreams/images/renders/'+data.Kits[i].image.name+'" alt="'+data.Kits[i].image.alt+'">');
            $("#kits").append('<br>');
